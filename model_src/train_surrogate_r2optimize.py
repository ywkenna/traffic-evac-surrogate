...
# [추가된 부분] Spearman 상관계수 계산을 위한 라이브러리 임포트
...
def pick_x_columns(df: pd.DataFrame, prefixes: list[str]) -> list[str]:
    cols = []
    for c in df.columns:
        if c in DROP_COLS or c in Y_COLS: continue
...
def main():
    ...
    X, Y_log, Y_orig, test_size=TEST_SIZE, random_state=RANDOM_SEED
    )
